VariableSelection <- function(maxent,
                              outdir,
                              gridfolder,
                              occurrencelocations,
                              backgroundlocations,
    ...
    cat(c("Test","Model","betamultiplier",beginningvariableset,"\n"),
        file = paste(outdir,"/VariableSelectionProcess.txt", sep=""),
        sep = "\t", fill = FALSE, labels = NULL, append = FALSE)

    cat(c("Model","betamultiplier","variables","samples","parameters",
          "loglikelihood","AIC","AICc","BIC","AUC.Test","AUC.Train",
          "AUC.Diff","\n"),
        file = paste(outdir,"/ModelPerformance.txt",sep=""),
        sep = "\t", fill = FALSE, labels = NULL, append = FALSE)
    ...
    for (b in betamultiplier){
        ...
        file.copy(from=occurrencesites,
                  to= paste(outdir,"/",
                            occurrencesitefilename,
                            "_VariableSubset.csv",sep=""),
                  overwrite=TRUE)
        file.copy(from=backgroundsites,
                  to= paste(outdir,"/",backgroundsitefilename,
                            "_VariableSubset.csv",sep=""),overwrite=TRUE)

        occurrencesites <- paste(outdir,"/",occurrencesitefilename,
                                 "_VariableSubset.csv",sep="")
        backgroundsites <- paste(outdir,"/",backgroundsitefilename,
                                 "_VariableSubset.csv",sep="")
        ...
        while(length(already.tested.variables)<length(variablenames)){
            ...
            MaxentVariableSelection:::Maxentrun(maxent = maxent,
                      outdir = outdir,
            ...
            AUCs <- MaxentVariableSelection:::MaxentAUC(outdir)

            variablecontributions <- MaxentVariableSelection:::Subsetselection(outdir)
